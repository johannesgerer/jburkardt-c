...
*/
{
  ...
  u_log = ( double * ) malloc ( order * sizeof ( double ) );
...
*/
{
  ...
  u = ( double * ) malloc ( order * sizeof ( double ) );
  u_rat = ( double * ) malloc ( order * sizeof ( double ) );
  weight = ( double * ) malloc ( order * sizeof ( double ) );
...
/*
  ...
  Parameters:
    ...
    Output, char *P00_TITLE, the title of the problem.
*/
{
  char *title;
...
*/
{
  ...
  for ( i = 0; i < n; i++ )
  {
    f[i] = exp ( -2.0 ) * cos ( 0.5 * pi * x[i] * x[i] );
  }
...
*/
{
  ...
  strcpy ( title, "Complementary error function" );
...
*/
{
  ...
  strcpy ( title, "Gamma(Z=5) function" );
...
*/
{
  ...
  strcpy ( title, "cos ( pi x / 2 ) / sqrt ( x )" );
...
*/
{
  ...
  exact = sqrt ( pi ) * cos ( 0.5 * atan ( pow ( 2.0, beta ) ) ) 
...
*/
{
  ...
  strcpy ( title, "exp ( - x / 2^beta ) * cos ( x ) / sqrt ( x )" );
...
*/
{
  ...
  {
    exact = ( 1.0 - beta ) * pi / ( pow ( 10.0, beta ) * sin ( pi * beta ) );
  }
  return exact;
}
/******************************************************************************/
...
*/
{
  ...
  strcpy ( title, "x^(beta-1) / ( 1 + 10 x )^2" );
...
*/
{
  ...
  strcpy ( title, "1 / ( 2^beta * ( ( x - 1 )^2 + (1/4)^beta ) * ( x - 2 ) )" );
