#!/bin/bash
#
gcc -c -g -I/$HOME/include test_int_laguerre_prb.c >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling test_int_laguerre_prb.c"
  exit
fi
rm compiler.txt
#
gcc test_int_laguerre_prb.o /$HOME/libc/$ARCH/test_int_laguerre.o -lm
if [ $? -ne 0 ]; then
