...
/*
  ...
  Parameters:
    ...
    Input, char *FILENAME, a common filename prefix for 
...
*/
  ...
  for ( j = 0; j < n; j++ )
  {
    fprintf ( data_unit, "  %24.16g  %24.16g\n", xy[0+j*2], xy[1+j*2] );
  }
  fclose ( data_unit );
  printf ( "\n" );
...
*/
  strcpy ( command_filename, filename );
  ...
  fprintf ( command_unit, 
    "plot [-1:+1] [-1:+1] '%s' using 1:2 with points lt 3 pt 3\n", 
    data_filename );
  fclose ( command_unit );
...
/*
  ...
  Parameters:
    ...
    Output, double W[((L+1)*(L+2))/2], the quadrature weights.
*/
{
  int n;
  double *w;
  ...
  else
  {
    ...
    fprintf ( stderr, "  Illegal value of L = %d\n", l );
    fprintf ( stderr, "  Legal values are 0 through 10.\n" );
    exit ( 1 );
  }
/*
  Reverse data to match published information.
...
*/
  te1 = ( double * ) malloc ( lp2h * lp2h * sizeof ( double ) );
  ...
  to1 = ( double * ) malloc ( lp2h * lp1h * sizeof ( double ) );
  ...
  te2 = ( double * ) malloc ( lp2h * lp3h * sizeof ( double ) );
  ...
  to2 = ( double * ) malloc ( lp2h * lp2h * sizeof ( double ) );
...
*/
  mom = ( double * ) malloc ( lp2h * lp2h * sizeof ( double ) );
...
*/
  tmtoe = ( double * ) malloc ( lp2h * lp2h * sizeof ( double ) );
  ...
  tmteo = ( double * ) malloc ( lp3h * lp1h * sizeof ( double ) );
...
*/
  w1 = ( double * ) malloc ( lp2h * lp2h * sizeof ( double ) );
  ...
  w2 = ( double * ) malloc ( lp3h * lp1h * sizeof ( double ) );
