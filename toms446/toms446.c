...
*/
{
  ...
  w2 = ( double * ) malloc ( npl * sizeof ( double ) );
...
/*
  ...
  Discussion:
    ...
    An example of a routine to compute the function values is:
      ...
      {
        double *val;
        val = ( double * ) malloc ( 2 * sizeof ( double ) );;
        val[0] = sin(a);
        val[1] = cos(a);
        return val;
      }
...
*/
{
  ...
  gc = ( double * ) malloc ( 2 * n * sizeof ( double ) );
  for ( k = 1; k <= 2 * n; k++ )
  {
    fk = ( double ) ( k - 1 );
    gc[k-1] = cos ( fk * pen );
...
*/
{
  ...
  xnvse = ( double * ) malloc ( npl * sizeof ( double ) );
...
*/
{
  ...
  ww = ( double * ) malloc ( npl * sizeof ( double ) );
  w2 = ( double * ) malloc ( npl * sizeof ( double ) );
