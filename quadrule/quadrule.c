...
*/
{
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "BASHFORTH_SET - Fatal error!\n" );
    fprintf ( stderr, "  Illegal value of N = %d\n", n );
    fprintf ( stderr, "  Legal values are 1:10, 12, 14, 16, 18 or 20.\n" );
    exit ( 1 );
  }
...
/*
  ...
  Discussion:
    ...
    The quadrature rule:
      ...
      Sum ( 1 <= I <= ORDER ) WEIGHT(I) * BDF**(I-1) FUNC ( 0 )
...
*/
{
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "HERMITE_GENZ_KEISTER_SET - Fatal error!\n" );
...
/*
  ...
  Parameters:
    ...
    Input, int ORDER, the order.  
    ORDER must be between 1 and 10 or 12, 14, 16, 18 or 20.
...
*/
{
  ...
  nc_compute ( n, x_min, x_max, xtab, weight );
...
/*
  ...
  Parameters:
    ...
    Input/output, double X[ORDER], the abscissas.

    Input/output, double W[ORDER], the weights.
*/
{
  int i;
