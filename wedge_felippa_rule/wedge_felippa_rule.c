...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "WEDGE_INTEGRAL - Fatal error!\n" );
    fprintf ( stderr, "  EXPON[2] = -1 is not a legal input.\n" );
    exit ( 1 );
  }
  else if ( ( expon[2] % 2 ) == 1 )
  {
...
*/
{
  ...
  line_w = ( double * ) malloc ( line_order * sizeof ( double ) );
  line_x = ( double * ) malloc ( line_order * sizeof ( double ) );
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "WEDGE_RULE - Fatal error!\n" );
    fprintf ( stderr, "  Illegal value of LINE_ORDER.\n" );
    exit ( 1 );
  }

  triangle_w = ( double * ) malloc ( abs(triangle_order) * sizeof ( double ) );
  triangle_xy = ( double * ) malloc (  2 * abs(triangle_order) * sizeof ( double ) );
  ...
  else
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "WEDGE_RULE - Fatal error!\n" );
    fprintf ( stderr, "  Illegal value of TRIANGLE_ORDER.\n" );
    exit ( 1 );
  }
