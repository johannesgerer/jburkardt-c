...
*/
{
  ...
  pols = ( double * ) malloc ( ( n + 1 ) * sizeof ( double ) );
...
/*
  Compute F2S.
*/
  f2s = ( double * ) malloc ( ( degree + 1 ) * ( degree + 1 ) * sizeof ( double ) );
...
/*
  Compute F3S.
*/
  f3s = ( double * ) malloc ( ( degree + 1 ) * ( degree + 1 ) * sizeof ( double ) );
...
*/
  nvals = ( ( degree + 1 ) * ( degree + 2 ) * ( degree + 3 ) ) / 6;
  fvals = ( double * ) malloc ( nvals * sizeof ( double ) );
...
*/
{
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "TETRAHEDRON_ARBQ - Fatal error\n" );
...
void tetrahedron_arbq_gnuplot ( int n, double x[], char *header )
...
*/
{
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "TETRAHEDRON_ARBQ_SIZE - Fatal error!\n" );
    fprintf ( stderr, "  Illegal value of DEGREE.\n" );
    exit ( 1 );
  }
