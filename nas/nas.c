...
void cfft2d1 ( int is, int m, int m1, int n, double complex x[], 
  double complex w[], int ip[] );
void cfft2d2 ( int is, int m, int m1, int n, double complex x[], 
  double complex w[], int ip[] );
...
void gmtry ( int nb, int nw, int nwall[], double complex proj[], double rmatrx[], 
  double complex wall[], double xmax[], double complex zcr[] );
void emitst ( double *er, double *fp, double *tm );
void emit ( int nb, int nvm, int nw, double cp[], double dpds[], 
  double complex expmz[], double complex expz[], double complex force[], 
  double gamma[], int nwall[], double ps[], double psi[], 
  double complex refpt[], double rhs[], double rmatrx[], double rmom[], 
  double complex wall[], double complex z[], double complex zcr[] );
...
/*
  Results.
*/
  *er = abs ( ( creal ( x[18+18*m1] ) - ans ) / ans );
...
*/
  ...
  for ( l = 1; l <= nb; l++ )
  {
    ...
    {
      k2 = 1 + ( k % nwall[l-1] );
      arcl = arcl + cabs ( wall[k-1+(l-1)*nw] - wall[k2-1+(l-1)*nw] );
    }
  }
/*
  Compute core radius.
...
*/
  for ( l = 1; l <= nb; l++ )
  {
    for ( k = 1; k <= nwall[l-1]; k++ )
    {
      ...
      zcr[k-1+(l-1)*nw] = wall[k-1+(l-1)*nw] + r0 * I / cabs ( zz ) * zz;
    }
/*
...
*/
    xmax[l-1] = creal ( zcr[0+(l-1)*nw] );
    ls = 0;

    for ( k = 1; k <= nwall[l-1]; k++ )
    {
      ymin = r8_min ( ymin, cimag ( zcr[k-1+(l-1)*nw] ) );
      ymax = r8_max ( ymax, cimag ( zcr[k-1+(l-1)*nw] ) );
      xmax[l-1] = r8_max ( xmax[l-1], creal ( zcr[k-1+(l-1)*nw] ) );
      kp = 1 + ( k % nwall[l-1] );

      if ( 0.0 < creal ( ( zcr[kp-1+(l-1)*nw] - zcr[k-1+(l-1)*nw] ) * 
  ...
  for ( l = 1; l <= nb; l++ )
  {
    for ( k = 1; k <= nwall[l-1]; k++ )
    {
      proj[k-1+(l-1)*nw] = zcr[k-1+(l-1)*nw] - period * I *
        ( ( int ) ( 5.0 + ( cimag ( zcr[k-1+(l-1)*nw] ) - ylimit ) 
        / period ) - 5.0 );
    }
  }
/*
  ...
  for ( l1 = 1; l1 <= nb; l1++ )
  {
    ...
    for ( l2 = 1; l2 <= nb; l2++ )
    {
      ...
      for ( j = 1; j <= nwall[l2-1]; j++ )
      {
        ...
        z1 = cexp ( ( wall[0+(l1-1)*nw] - zcr[j-1+(l2-1)*nw] ) * pidp );
        z1 = z1 - 1.0 / z1;
        dum = sig2 + pow ( creal ( z1 ), 2 ) + pow ( cimag ( z1 ), 2 );
        for ( i = 2; i <= nwall[l1-1]; i++ )
        {
          zi = cexp ( ( wall[i-1+(l1-1)*nw] - zcr[j-1+(l2-1)*nw] ) * pidp );
          zz = zi - 1.0 / zi;
          rmatrx[i0+i-1+(j0+j-1)*nw*nb] = -0.25 / pi * log ( dum / 
            ( sig2 + pow ( creal ( zz ), 2 ) + pow ( cimag ( zz ), 2 ) ) );
        }
      }
      j0 = j0 + nwall[l2-1];
    }
...
*/
  ...
  for ( i = 0; i < nv; i++ )
  {
    expz[i] = cexp ( z[i] * pidp );
    expmz[i] = 1.0 / expz[i];
  }
  
  i0 = 0;
  cupst = pow ( creal ( uupstr ), 2 ) + pow ( cimag ( uupstr ), 2 );

  for ( l = 0; l < nb; l++ )
  {
    for ( k = 0; k < nwall[l]; k++ )
    {
      expwkl = cexp ( wall[k+l*nw] * pidp );
      expmwk = 1.0 / expwkl;
      ...
      for ( i = 0; i < nv; i++ )
      {
        ...
        ps[i] = gamma[i] * log ( pow ( creal ( dum3 ), 2 ) + 
          pow ( cimag ( dum3 ), 2 ) + sig2 );
        sps = sps + ps[i];
      }
      psi[k] = cimag ( wall[k+l*nw] * conj ( uupstr + u0 * I ) ) 
        - sps * 0.25 / pi;
    }
/*
...
*/
      ...
      rmom[l] = rmom[l] + gamma[nv] * ( 
        pow ( creal ( z[nv] - refpt[l] ), 2 ) +
        pow ( cimag ( z[nv] - refpt[l] ), 2 ) );
