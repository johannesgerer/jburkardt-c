...
/*
  Purpose:
    ...
    SUPER_LU_D4 solves a sparse system read from a file using SGSSVX.

  Discussion:

    In this example, we show how the LU factors computed by one call to
    DGSSVX can be reused to solve a second linear system with the same
    matrix by calling DGSSVX with the appropriate options set.
