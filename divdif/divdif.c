...
/*
  ...
  abscissas to zero.
*/
  xtab1 = ( double * ) malloc ( ntab * sizeof ( double ) );
  diftab1 = ( double * ) malloc ( ntab * sizeof ( double ) );
...
void dif_print ( int ntab, double xtab[], double diftab[], char *title  )
...
*/
  poly_cof = ( double * ) malloc ( norder * sizeof ( double ) );
...
void r8poly_print ( int n, double poly_cof[], char *title  )
...
/*
  ...
  Parameters:
    ...
    Output, int R8VEC_DISTINCT is 1 if all N elements of X are distinct.
*/
{
  int i;
  int j;
...
*/
  xtab = ( double * ) malloc ( ( nroots + 1 ) * sizeof ( double ) );
  for ( i = 0; i < *nc-1; i++ )
  {
    xtab[i] = roots[i];
  }
