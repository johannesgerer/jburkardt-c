...
#  "matvec.oJOBID" where JOBID is a job id number assigned when the job is
#  submitted.
#
#MOAB -j oe
#
...
#
if [ $? -ne 0 ]; then
  echo "Errors compiling matvec_mpi.c"
  exit
fi
#
...
#
mpirun -np 8 ./matvec > matvec_fsu_output.txt
#
#  Clean up.
#
rm matvec
#
echo "Program output written to matvec_fsu_output.txt"
