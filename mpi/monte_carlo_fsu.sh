...
#  "monte_carlo.oJOBID" where JOBID is a job id number assigned when the job is
#  submitted.
#
#MOAB -j oe
#
...
#
mpicc monte_carlo_mpi.c
#
if [ $? -ne 0 ]; then
  echo "Errors compiling monte_carlo_mpi.c"
  exit
fi
#
...
#
mpirun -np 8 ./monte_carlo > monte_carlo_fsu_output.txt
#
#  Clean up.
#
rm monte_carlo
#
echo "Program output written to monte_carlo_fsu_output.txt"
