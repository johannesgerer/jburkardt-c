...
*/
{
  ...
  printf ( "  Final time      = %g\n", tf );
  printf ( "  Viscosity Mean  = %g\n", numean );
  printf ( "  Viscosity Var   = %g\n", nuvariance );
  printf ( "\n" );

  u1 = ( double * ) malloc ( (n+1)*(p+1) * sizeof ( double ) );
  u2 = ( double * ) malloc ( (n+1)*(p+1) * sizeof ( double ) );
...
*/
  strcpy ( output_filename, "burgers.history.txt" );
...
*/
  ...
  fclose ( output );
  printf ( "  Time history in \"%s\".\n", output_filename );
/*
  Compute the mean and variance.
*/
  umean = ( double * ) malloc ( ( n + 1 ) * sizeof ( double ) );
  uvariance = ( double * ) malloc ( ( n + 1 ) * sizeof ( double ) );
...
*/
  strcpy ( output_filename, "burgers.moments.txt" );
  output = fopen ( output_filename, "wt" );
  fprintf ( output, "X E[U] Var[U]\n" );
  for ( i = 0; i <= n; i++ )
  {
    fprintf ( output, "  %18g  %18g  %18g\n", x[i], umean[i], uvariance[i] );
  }
  fclose ( output );
  printf ( "  Moments written to \"%s\".\n", output_filename );

  strcpy ( output_filename, "burgers.modes.txt" );
  output = fopen ( output_filename, "wt" );
  fprintf ( output, "X U_0 ... U_P\n" );
  for ( i = 0; i <= n; i++ )
  {
    fprintf ( output, "  %20g", x[i] );
    for ( j = 0; j <= p; j++ )
    {
      fprintf ( output, "  %20g", u1[i+j*(n+1)] );
    }
    fprintf ( output, "\n" );
  }
  fclose ( output );
  printf ( "  Final modes written to \"%s\".\n", output_filename );
/*
  Free memory.
*/
  free ( u1 );
