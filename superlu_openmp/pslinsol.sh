...
#  Compile
#
gcc -c -g -fopenmp -I/$HOME/include pslinsol.c >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling pslinsol.c"
  exit
fi
rm compiler.txt
#
#  Link and load
#
gcc -fopenmp pslinsol.o -L/$HOME/lib/$ARCH -L/$HOME/libc/$ARCH -lsuperlu_openmp -lm -lblas
if [ $? -ne 0 ]; then
