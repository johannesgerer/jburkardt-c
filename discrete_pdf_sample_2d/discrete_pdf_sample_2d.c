...
double *discrete_cdf_to_xy ( int n1, int n2, double cdf[], int n, double u[],
  int *seed );
double *get_discrete_pdf_data1 ( int n1, int n2 );
double *get_discrete_pdf_data2 ( int n1, int n2 );
void get_discrete_pdf_size1 ( int *n1, int *n2 );
void get_discrete_pdf_size2 ( int *n1, int *n2 );
...
*/
{
  ...
  timestamp ( );
  printf ( "\n" );
  printf ( "DISCRETE_PDF_SAMPLE_2D:\n" );
  printf ( "  C version\n" );
  printf ( "  Generate sample data using a discrete PDF.\n" );
...
*/
  get_discrete_pdf_size1 ( &n1, &n2 );
  printf ( "  PDF data is on a %d by %d grid.\n", n1, n2 );
/*
...
*/
  xy = discrete_cdf_to_xy ( n1, n2, cdf, n, u, &seed );
/*
...
*/
{
  ...
  pdf = r8mat_copy_new ( n1, n2, pdf_save );
/*
...
*/
  ...
  printf ( "\n" );
  printf ( "  PDF data sums to %g\n", total );

  scale = 1.0 / total;

  r8mat_scale ( n1, n2, scale, pdf );
...
*/
{
  ...
  cdf = ( double * ) malloc ( n1 * n2 * sizeof ( double ) );
