...
double triangle01_monomial_integral ( int dim_num, int expon[] );
...
*/
{
  ...
  timestamp ( );
  printf ( "\n" );
  printf ( "TRIANGLE_EXACTNESS\n" );
  printf ( "  C version\n" );
  printf ( "\n" );
  printf ( "  Investigate the polynomial exactness of a quadrature\n" );
  printf ( "  rule for the triangle by integrating all monomials\n" );
  printf ( "  of a given degree.\n" );
  printf ( "\n" );
  printf ( "  The rule will be adjusted to the unit triangle.\n" );
/*
...
*/
  printf ( "\n" );
  printf ( "TRIANGLE_EXACTNESS: User input:\n" );
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "TRIANGLE_EXACTNESS - Fatal error!\n" );
    fprintf ( stderr, "  The quadrature abscissas must be two dimensional.\n" );
    exit ( 1 );
  }
...
*/
  ...
  if ( point_num3 != 3 )
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "TRIANGLE_EXACTNESS - Fatal error!\n" );
    fprintf ( stderr, "  The quadrature region file should have 3 lines.\n" );
    exit ( 1 );
  }

  r = r8mat_data_read ( quad_r_filename, dim_num, 3 );
/*
  Rescale the weights.
*/
...
*/
  x_ref = ( double * ) malloc ( 2 * point_num * sizeof ( double ) );

  triangle_order3_physical_to_reference ( r, point_num, x, x_ref );
/*
...
*/
  ...
  for ( degree = 0; degree <= degree_max; degree++ )
  {
    ...
    for ( ; ; )
    {
      ...
      quad_error = triangle01_monomial_quadrature ( dim_num, expon, point_num,
        x_ref, w );
