...
void timestamp ( );
void xml_mesh1d_write ( char *xml_filename, int m, int node_num, double node_x[],
...
void xml_mesh2d_write ( char *xml_filename, int m, int node_num, double node_x[],
  int element_order, int element_num, int element_node[] );
void xml_mesh3d_write ( char *xml_filename, int m, int node_num, double node_x[],
  int element_order, int element_num, int element_node[] );
...
/*
  ...
  Discussion:
    ...
    FEM_TO_XML converts a 1D, 2D or 3D mesh from FEM to XML format.
  ...
  Usage:
    ...
    * 'prefix'_elements.txt contains the element connectivity
...
*/
  if ( m == 1 )
  {
    xml_mesh1d_write ( xml_filename, m, node_num, node_x, element_order,
  ...
  {
    xml_mesh3d_write ( xml_filename, m, node_num, node_x, element_order,
      element_num, element_node );
  }
...
void xml_mesh1d_write ( char *xml_filename, int m, int node_num, 
...
void xml_mesh3d_write ( char *xml_filename, int m, int node_num, 
  double node_x[], int element_order, int element_num, int element_node[] )
...
*/
  ...
  fprintf ( xml, "\n" );
  fprintf ( xml, "<dolfin xmlns:dolfin=\"http://www.fenics.org/dolfin/\">\n" );
  fprintf ( xml, "  <mesh celltype=\"tetrahedron\" dim=\"%d\">\n", m );

  fprintf ( xml, "    <vertices size=\"%d\">\n", node_num );
  for ( node = 0; node < node_num; node++ )
  {
    fprintf ( xml, 
      "      <vertex index =\"%d\" x =\"%g\" y =\"%g\" z =\"%g\"/>\n", 
      node, node_x[0+node*m], node_x[1+node*m], node_x[2+node*m] );
  }
  fprintf ( xml, "    </vertices>\n" );
