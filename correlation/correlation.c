...
{
  double *c;
  char command_filename[] = "brownian_plots_commands.txt";
  FILE *command_unit;
  char data_filename[] = "brownian_plots_data.txt";
  FILE *data_unit;
  int i;
  int j;
  int n = 101;
  ...
  for ( i = 0; i < n; i++ )
  {
    fprintf ( data_unit, "  %14.6g", s[i] );
    for ( j = 0; j < n2; j++ )
    {
      fprintf ( data_unit, "  %14.6g", c[i+j*n] );
    }
    fprintf ( data_unit, "\n" );
  }
  fclose ( data_unit );
  ...
  fprintf ( command_unit, "set output \"brownian_plots.png\"\n" );
  fprintf ( command_unit, "set title 'Brownian correlation C(S,T), S = 0.25, 1.5, 2.5, 3.75'\n" );
  fprintf ( command_unit, "set xlabel 'S'\n" );
  fprintf ( command_unit, "set ylabel 'C(s,t)'\n" );
  fprintf ( command_unit, "set grid\n" );
  fprintf ( command_unit, "set style data lines\n" );
  fprintf ( command_unit, "plot \"%s\" using 1:2 lw 3 linecolor rgb 'blue',\\\n", data_filename );
  fprintf ( command_unit, "     \"%s\" using 1:3 lw 3 linecolor rgb 'blue',\\\n", data_filename );
  fprintf ( command_unit, "     \"%s\" using 1:4 lw 3 linecolor rgb 'blue',\\\n", data_filename );
  fprintf ( command_unit, "     \"%s\" using 1:5 lw 3 linecolor rgb 'blue'\n", data_filename );
...
*/
{
  ...
  fprintf ( command_unit, "#\n" );
  fprintf ( command_unit, "set term png\n" );
  fprintf ( command_unit, "set output \"%s_plot.png\"\n", header );
...
*/
{
  ...
  strcpy ( data_filename, header );
  strcat ( data_filename, "_plots_data.txt" );

  data_unit = fopen ( data_filename, "wt" );
  for ( i = 0; i < n; i++ )
  {
    fprintf ( data_unit, "  %14.6g", rho[i] );
    for ( j = 0; j < n2; j++ )
    {
      fprintf ( data_unit, "  %14.6g", c[i+j*n] );
    }
  ...
  strcpy ( command_filename, header );
  strcat ( command_filename, "_plots_commands.txt" );
  ...
  fprintf ( command_unit, "#\n" );
  fprintf ( command_unit, "set term png\n" );
  fprintf ( command_unit, "set output \"%s_plots.png\"\n", header );
  fprintf ( command_unit, "set xlabel 'Rho'\n" );
  fprintf ( command_unit, "set ylabel 'Correlation(Rho)'\n" );
  fprintf ( command_unit, "set title '%s'\n", title );
  fprintf ( command_unit, "set grid\n" );
  fprintf ( command_unit, "set style data lines\n" );
  fprintf ( command_unit, "set key off\n" );
  if ( n2 == 1 )
  {
    fprintf ( command_unit, "plot '%s' using 1:2 lw 3\n", data_filename );
  }
  else
  {
    fprintf ( command_unit, "plot '%s' using 1:2 lw 3, \\\n", data_filename );
    for ( i = 2; i < n2; i++ )
    {
      fprintf ( command_unit, "     '%s' using 1:%d lw 3, \\\n", data_filename, i + 1 );
    }
    fprintf ( command_unit, "     '%s' using 1:%d lw 3\n", data_filename, n2 + 1 );
  }
  fprintf ( command_unit, "quit\n" );
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "CORRELATION_TO_COVARIANCE - Fatal error!\n" );
    fprintf ( stderr, "  Input matrix C fails symmetry test with error %g\n", error_frobenius );
    exit ( 1 );
  }
/*
...
*/
  for ( i = 0; i < n; i++ )
  {
    ...
    if ( tol < e )
    {
      fprintf ( stderr, "\n" );
      fprintf ( stderr, "CORRELATION_TO_COVARIANCE - Fatal error\n" );
      fprintf ( stderr, "  Input matrix C has non-unit diagonal entries.\n" );
      fprintf ( stderr, "  Error on row %d is %g\n", i, e );
      exit ( 1 );
    }
  }
/*
...
*/
  ...
  if ( c_min < - 1.0 - tol )
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "CORRELATION_TO_COVARIANCE - Fatal error\n" );
    fprintf ( stderr, "  Input matrix C has entries less than -1.0\n" );
    exit ( 1 );
  }
  ...
  if ( 1.0 + tol < c_max )
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "CORRELATION_TO_COVARIANCE - Fatal error\n" );
    fprintf ( stderr, "  Input matrix C has entries greater than +1.0\n" );
    exit ( 1 );
  }
/*
  Form K.
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "COVARIANCE_TO_CORRELATION - Fatal error\n" );
    fprintf ( stderr, "  Input matrix K fails symmetry test with error %g\n", error_frobenius );
    exit ( 1 );
  }
/*
...
*/
  ...
  if ( tol < e )
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "COVARIANCE_TO_CORRELATION - Fatal error\n" );
    fprintf ( stderr, "  Input matrix K fails K(I,J)^2 <= K(I,I)*K(J,J)\n" );
    exit ( 1 );
  }
/*
  Get the diagonal.
...
*/
  ...
  if ( sigma_min <= 0.0 )
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "COVARIANCE_TO_CORRELATION - Fatal error!\n" );
    fprintf ( stderr, "  Input matrix K has nonpositive diagonal entry = %g\n", sigma_min );
    exit ( 1 );
  }
/*
...
void paths_plot ( int n, int n2, double rho[], double x[], char *header, 
  char *title )
...
*/
{
  ...
  strcpy ( data_filename, header );
  strcat ( data_filename, "_path_data.txt" );
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    for ( j = 0; j < n2; j++ )
    {
      fprintf ( data_unit, "  %14.6g", x[i+j*n] );
    }
    fprintf ( data_unit, "\n" );
  }
  fclose ( data_unit );
  ...
  strcpy ( command_filename, header );
  strcat ( command_filename, "_path_commands.txt" );
  ...
  fprintf ( command_unit, "#\n" );
  fprintf ( command_unit, "set term png\n" );
  fprintf ( command_unit, "set output \"%s_paths.png\"\n", header );
  fprintf ( command_unit, "set xlabel 'Rho'\n" );
  fprintf ( command_unit, "set ylabel 'X(Rho)'\n" );
  fprintf ( command_unit, "set title '%s'\n", title );
  fprintf ( command_unit, "set grid\n" );
  fprintf ( command_unit, "set style data lines\n" );
  fprintf ( command_unit, "set key off\n" );
  ...
  else
  {
    ...
    for ( i = 2; i < n2; i++ )
    {
      fprintf ( command_unit, "     '%s' using 1:%d, \\\n", data_filename, i + 1 );
    }
    fprintf ( command_unit, "     '%s' using 1:%d\n", data_filename, n2 + 1 );
  }
  fprintf ( command_unit, "quit\n" );
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "SAMPLE_PATHS_CHOLESKY - Fatal error!\n" );
    fprintf ( stderr, "  The correlation matrix is not\n" );
    fprintf ( stderr, "  symmetric nonnegative definite.\n" );
    exit ( 1 );
  }
/*
...
*/
  ...
  {
    printf ( "\n" );
    printf ( "SAMPLE_PATHS_EIGEN - Warning!\n" );
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "SAMPLE_PATHS2_CHOLESKY - Fatal error!\n" );
    fprintf ( stderr, "  The correlation matrix is not\n" );
    fprintf ( stderr, "  symmetric nonnegative definite.\n" );
    exit ( 1 );
  }
