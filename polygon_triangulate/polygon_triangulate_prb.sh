#!/bin/bash
#
gcc -c -I/$HOME/include polygon_triangulate_prb.c >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling polygon_triangulate_prb.c"
  exit
fi
rm compiler.txt
#
gcc polygon_triangulate_prb.o /$HOME/libc/$ARCH/polygon_triangulate.o -lm
if [ $? -ne 0 ]; then
