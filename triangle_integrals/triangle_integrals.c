...
*/
{
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "POLY_POWER_LINEAR - Fatal error!\n" );
    fprintf ( stderr, "  D1 < 0.\n" );
    exit ( 1 );
  }
...
void poly_print ( int d, double p[], char *title )
...
/*
  ...
  Parameters:
    ...
    Input, char *TITLE, a title string.
*/
{
  int i;
  int j;
...
*/
  d3 = d1 + d2;
  m3 = ( ( d3 + 1 ) * ( d3 + 2 ) ) / 2;
  p3 = ( double * ) malloc ( m3 * sizeof ( double ) );
...
*/
  d1 = 1;
  m1 = ( ( d1 + 1 ) * ( d1 + 2 ) ) / 2;
  p1 = ( double * ) malloc ( m1 * sizeof ( double ) );
  p1[0] = a;
  p1[1] = b;
  p1[2] = c;
