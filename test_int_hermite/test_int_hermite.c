...
*/
{
  ...
  {
    value = ( double ) ( i4_factorial2 ( n - 1 ) ) * sqrt ( pi ) 
      / pow ( 2.0, n / 2 );
  }
...
*/
{
  ...
  else
  {
    ...
    fprintf ( stderr, "  Illegal problem number = %d", problem );
    exit ( 1 );
  }
...
*/
{
  double *f_vec;
  int option;
  double pi_sqrt = 1.7724538509055160273;
  double result;
  int seed;
  double weight;
  double *x_vec;
  ...
  weight = ( double ) ( order ) / pi_sqrt / sqrt ( 2.0 );
...
*/
    ...
    {
      fprintf ( stdout, "\n" );
      fprintf ( stdout, "P00_TURING - Warning!\n" );
      fprintf ( stdout, "  Number of steps exceeded N_TOO_MANY = %d\n", n_too_many );
      break;
    }
...
*/
{
  ...
  strcpy ( title, "exp(-x*x) * cos(2*omega*x)" );
...
*/
{
  ...
  strcpy ( title, "1/( (1+x^2) sqrt(4+3x^2) )" );
...
*/
{
  ...
  {
    exact = ( double ) ( i4_factorial2 ( m - 1 ) ) * sqrt ( pi ) 
      / pow ( 2.0, m / 2 );
  }
...
*/
{
  ...
  if ( action == 'S' || action == 's' )
  {
    ...
    {
      fprintf ( stderr, "\n" );
      fprintf ( stderr, "P06_PARAM - Fatal error!\n" );
      fprintf ( stderr, "  Parameter M must be greater than -1.\n" );
      exit ( 1 );
    }
    m = *value;
  }
  ...
  else
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "P06_PARAM - Fatal error!\n" );
    fprintf ( stderr, "  Unrecognized value of ACTION = \"%s\".\n", action );
    exit ( 1 );
  }
  return;
}
...
*/
{
  ...
  strcpy ( title, "x^2 cos ( x ) exp(-x*x)" );
...
*/
{
  ...
  strcpy ( title, "sqrt(1+x*x/2) * exp(-x*x/2)" );
