...
int simdo ( int qind, int qfor, int iprod[], int kdim, int *jsub, int ivec[] )
...
*/
  if ( qind )
  {
    if ( iprod[kdim-1] < *jsub )
    {
      ifault = 1;
      printf ( "\n" );
      printf ( "SIMDO - Fatal error!\n" );
      printf ( "  JSUB is out of bounds.\n" );
      exit ( ifault );
    }
...
*/
  else
  {
    ...
    if ( iprod[0] < ivec[0] )
    {
      ...
      printf ( "\n" );
      printf ( "SIMDO - Fatal error!\n" );
      printf ( "  An entry of IVEC is out of bounds.\n" );
      exit ( ifault );
    }
