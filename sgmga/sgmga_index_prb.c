...
void sgmga_index_tests ( void );
void sgmga_index_test ( int dim_num, double importance[], double level_weight[], 
...
/*
  Purpose:
    ...
    MAIN is the main program for SGMGA_INDEX_PRB.
...
*/
{
  ...
  printf ( "\n" );
  printf ( "SGMGA_INDEX_TESTS\n" );
  printf ( "  Call SGMGA_INDEX_TEST with various arguments.\n" );
/*
...
void sgmga_index_test ( int dim_num, double importance[], 
...
*/
{
  ...
  printf ( "\n" );
  printf ( "SGMGA_INDEX_TEST\n" );
  printf ( "  SGMGA_INDEX returns index and order vectors that\n" );
  printf ( "  identify each point in a multidimensional sparse grid \n" );
  printf ( "  with mixed factors.\n" );
  printf ( "\n" );
  ...
  for ( dim = 0; dim < dim_num; dim++ )
  {
    ...
    {
      printf ( "\n" );
      printf ( "SGMGA_INDEX_TEST - Fatal error!\n" );
  ...
  for ( level_max = level_max_min; level_max <= level_max_max; level_max++ )
  {
    ...
    for ( point = 0; point < point_num; point++ )
    {
      ...
      for ( dim = 0; dim < dim_num; dim++ )
      {
        printf ( "  %3d / %3d", sparse_index[dim+point*dim_num],
