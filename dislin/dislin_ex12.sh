#!/bin/bash
#
gcc -c -I/usr/local/dislin dislin_ex12.c >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling dislin_ex12.c"
  exit
fi
rm compiler.txt
#
gcc dislin_ex12.o -L/usr/local/dislin -ldislin -L/opt/local/lib -lXm -lm
if [ $? -ne 0 ]; then
