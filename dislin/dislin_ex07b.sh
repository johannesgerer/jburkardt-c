#!/bin/bash
#
gcc -c -I/usr/local/dislin dislin_ex07b.c >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling dislin_ex07b.c"
  exit
fi
rm compiler.txt
#
gcc dislin_ex07b.o -L/usr/local/dislin -ldislin -L/opt/local/lib -lXm -lm
if [ $? -ne 0 ]; then
