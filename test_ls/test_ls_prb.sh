#!/bin/bash
#
gcc -c -g -I/$HOME/include test_ls_prb.c >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling test_ls_prb.c"
  exit
fi
rm compiler.txt
#
gcc test_ls_prb.o /$HOME/libc/$ARCH/test_ls.o /$HOME/libc/$ARCH/r8lib.o -lm
if [ $? -ne 0 ]; then
  echo "Errors linking and loading test_ls_prb.o"
  exit
fi
#
rm test_ls_prb.o
#
mv a.out test_ls_prb
./test_ls_prb > test_ls_prb_output.txt
if [ $? -ne 0 ]; then
  echo "Errors running test_ls_prb."
  exit
fi
rm test_ls_prb
#
echo "Program output written to test_ls_prb_output.txt"
