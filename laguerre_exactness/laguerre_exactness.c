...
*/
{
  ...
  timestamp ( );
  printf ( "\n" );
  printf ( "LAGUERRE_EXACTNESS\n" );
  printf ( "  C version\n" );
  printf ( "\n" );
  printf ( "  Investigate the polynomial exactness of a Gauss-Laguerre\n" );
  printf ( "  quadrature rule by integrating exponentially weighted\n" );
  printf ( "  monomials up to a given degree over the [0,+oo) interval.\n" );
  printf ( "\n" );
  printf ( "  The rule may be defined on another interval, [A,+oo)\n" );
  printf ( "  in which case it is adjusted to the [0,+oo) interval.\n" );
/*
...
*/
  ...
  {
    printf ( "\n" );
    printf ( "OPTION chooses the standard or modified rule.\n" );
    printf ( "0: standard rule for integrating exp(-x)*f(x)\n" );
    printf ( "1: modified rule for integrating         f(x)\n" );
    scanf ( "%d", &option );
  }
/*
  Summarize the input.
*/
  printf ( "\n" );
  printf ( "LAGUERRE_EXACTNESS: User input:\n" );
  ...
  if ( option == 0 )
  {
    printf ( "  OPTION = 0, integrate exp(-x)*f(x)\n" );
  }
  else
  {
    printf ( "  OPTION = 1, integrate         f(x)\n" );
  }
/*
  Read the X file.
*/
  ...
  {
    printf ( "\n" );
    printf ( "LAGUERRE_EXACTNESS - Fatal error!\n" );
...
*/
  ...
  if ( option == 0 )
  {
    ...
    printf ( "    Integral ( A <= x < +oo ) exp(-x) f(x) dx\n" );
    printf ( "    is to be approximated by\n" );
    printf ( "    sum ( 1 <= I <= ORDER ) w(i) * f(x(i)).\n" );
  }
  else
  {
    printf ( "  Modified rule:\n" );
    printf ( "    Integral ( A <= x < +oo )         f(x) dx\n" );
    printf ( "    is to be approximated by\n" );
    printf ( "    sum ( 1 <= I <= ORDER ) w(i) * f(x(i)).\n" );
  }
  printf ( "\n" );
...
*/
  printf ( "\n" );
  printf ( "  A Gauss-Laguerre rule would be able to exactly\n" );
