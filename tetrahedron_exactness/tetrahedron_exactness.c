...
double tet01_monomial_integral ( int dim_num, int expon[] );
double tet01_monomial_quadrature ( int dim_num, int expon[], int point_num,
...
*/
{
  ...
  timestamp ( );
  printf ( "\n" );
  printf ( "TETRAHEDRON_EXACTNESS\n" );
  printf ( "  C version\n" );
  printf ( "\n" );
  printf ( "  Investigate the polynomial exactness of a quadrature\n" );
  printf ( "  rule for the tetrahedron by integrating all monomials\n" );
  printf ( "  of a given degree.\n" );
  printf ( "\n" );
  printf ( "  The rule will be adjusted to the unit tetrahedron.\n" );
/*
...
*/
  printf ( "\n" );
  printf ( "TETRAHEDRON_EXACTNESS: User input:\n" );
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "TETRAHEDRON_EXACTNESS - Fatal error!\n" );
    fprintf ( stderr, "  The quadrature abscissas must be 3 dimensional.\n" );
    exit ( 1 );
  }
...
/*
  Read the R file.
*/
  r8mat_header_read ( quad_r_filename, &dim_num3, &point_num3 );
  ...
  if ( point_num3 != 4 )
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "TETRAHEDRON_EXACTNESS - Fatal error!\n" );
    fprintf ( stderr, "  The quadrature region file should have 4 lines.\n" );
    exit ( 1 );
  }

  r = r8mat_data_read ( quad_r_filename, dim_num, 4 );
/*
  Rescale the weights.
*/
...
*/
  x_ref = ( double * ) malloc ( dim_num * point_num * sizeof ( double ) );

  tetrahedron_order4_physical_to_reference ( r, point_num, x, x_ref );
/*
...
*/
  ...
  for ( degree = 0; degree <= degree_max; degree++ )
  {
    ...
    for ( ; ; )
    {
      ...
      quad_error = tet01_monomial_quadrature ( dim_num, expon, point_num, x_ref,
        w );
...
*/
  scale = tet01_monomial_integral ( dim_num, expon );
/*
...
*/
  volume = 1.0 / 6.0;
  quad = volume * r8vec_dot_product ( point_num, weight, value ) / scale;
/*
  Error:
*/
  exact = 1.0;
