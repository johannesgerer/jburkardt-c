  ...
  for ( j = 0; j < k; j++ )
  {
    if ( alpha[j] <= 0.0 )
    {
      *ifault = 6;
      ...
      fprintf ( stderr, "  ALPHA(J) = %g\n", alpha[j] );
      fprintf ( stderr, "  but ALPHA(J) must be positive.\n" );
      exit ( 1 );
    }
  }
/*
    ...
    for ( j = 0; j < k; j++ )
    {
      if ( alpha[j] <= 0.0 )
      {
        *ifault = 6;
        ...
        fprintf ( stderr, "  Computed ALPHA[J] <= 0.\n" );
        fprintf ( stderr, "  J = %d\n", j );
        fprintf ( stderr, "  ALPHA[J] = %g\n", alpha[j] );
        exit ( 1 );
      }
...
*/
  ...
  a_sum = ( double * ) malloc ( comp_num * sizeof ( double ) );
...
/*
  Sample density COMP.
*/
  x = ( double * ) malloc ( elem_num * sizeof ( double ) );
